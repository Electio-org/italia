...
function currentMapRenderKey() {
  return createCurrentMapRenderKey(state);
}

function activeMunicipalityFeatures() {
  return mapActiveMunicipalityFeatures(state, geometryJoinKey);
}
...
function hitTestCanvasMap(event) {
  return hitTestMapCanvas(state, els.mapCanvas, event);
}
...
let mapTooltipController = null;

function scheduleHoverTooltip(event, feature, row) {
  mapTooltipController?.scheduleHoverTooltip(event, feature, row);
}

function showTooltip(event, feature, row, options = {}) {
  mapTooltipController?.showTooltip(event, feature, row, options);
}

function hideTooltip(force = false) {
  mapTooltipController?.hideTooltip(force);
}
...
function zoomToSelectedMunicipality(options = {}) {
  ...
  const x = (x0 + x1) / 2;
  const y = (y0 + y1) / 2;
  const scale = Math.max(1, Math.min(MAP_MAX_ZOOM, MUNICIPALITY_FOCUS_FILL / Math.max(dx / width, dy / height)));
...
async function init() {
  ...
  mapTooltipController = createMapTooltipController({
    state,
    getTooltipElement: () => els.tooltip,
    getWrapperElement: () => q('map-wrapper'),
    geometryJoinKey,
    escapeHtml,
    fmtPct,
    fmtPctSigned,
